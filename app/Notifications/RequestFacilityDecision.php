...
class RequestFacilityDecision extends Notification implements ShouldQueue
{
    ...
    public function toFcm($notifiable): FcmMessage
    {
        ...
        return FcmMessage::create()
            ->notification((new FcmNotification())->title($this->requestTitle)->body($body)->image('/FRAI.png'))
            ->data(['url' => (string) $this->url, 'category' => 'facility_decision', 'status' => (string) $this->status->value, 'facility' => (string) $this->facilityName]);
    }
